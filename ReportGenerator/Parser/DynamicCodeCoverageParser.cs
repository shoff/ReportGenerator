namespace Palmmedia.ReportGenerator.Parser
{
    ...
    public class DynamicCodeCoverageParser : ParserBase
    {
        ...
        public DynamicCodeCoverageParser(XContainer report)
        {
            ...
            var modules = report.Descendants("module").OrderBy(m => m.Attribute("name").Value).ToArray();

            // Parallel.ForEach(modules, assembly => this.AddAssembly(ProcessAssembly(assembly)));
            foreach (var assembly in modules)
            {
                var processedAssembly = this.ProcessAssembly(assembly);
        ...
        internal Assembly ProcessAssembly(XElement module)
        {
            var assemblyName = module.Attribute("name").Value;

            logger.DebugFormat("  " + Resources.CurrentAssembly, assemblyName);

            var classNames =
                module.Elements("functions").Elements("function").Select(f => f.Attribute("type_name").Value).Where(
                    c =>
                    !c.Contains("__") && !c.Contains("<>") && !c.Contains(".")
                    && !c.StartsWith("$", StringComparison.OrdinalIgnoreCase)).Distinct().OrderBy(name => name).ToArray();
            ...
            {
                var processedClass = this.ProcessClass(module, assembly, className);
        ...
        internal Class ProcessClass(XElement module, Assembly assembly, string className)
        {
            var fileIdsOfClass =
                module.Elements("functions").Elements("function").Where(c => c.Attribute("type_name").Value.Equals(className)).Elements
                    ("ranges").Elements("range").Select(r => r.Attribute("source_id").Value).Distinct();
            ...
            {
                var file = files.First(f => f.Attribute("id").Value == fileId).Attribute("path").Value;
                processClass.AddFile(ProcessFile(module, fileId, processClass, file));
            }
        ...
        internal CodeFile ProcessFile(XElement module, string fileId, Class @class, string filePath)
        {
            ...
            if (linesOfFile.Length > 0)
            {
                ...
                {
                    for (var lineNumber = seqpnt.LineNumberStart; lineNumber <= seqpnt.LineNumberEnd; lineNumber++)
                    {
                        coverage[lineNumber] = coverage[lineNumber] == -1
                                                   ? seqpnt.Coverage
                                                   : Math.Min(coverage[lineNumber] + seqpnt.Coverage, 1);
                    }
                }
            }
        ...
        /// <param name="metricClass">The class.</param>
        internal void SetMethodMetrics(IEnumerable<XElement> methods, Class metricClass)
        {
            foreach (var method in methods)
            {
                ...
                metricClass.AddMethodMetric(new MethodMetric(methodName, metrics));
            }
        }
    }
}