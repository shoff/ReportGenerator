namespace Palmmedia.ReportGenerator.Parser.Preprocessing
{
    ...
    public class PartCover23ReportPreprocessor : ReportPreprocessorBase
    {
        ...
        {
            var filenameByFileIdDictionary = this.Report.Descendants("File").ToDictionary(
                f => f.Attribute("id").Value, 
                f => f.Attribute("url").Value);
        ...
        {
            var document = filesContainer as XDocument;
        ...
        {
            for (var i = elementPosition.Start; i <= elementPosition.End; i++)
            {
                var seqpnt = new XElement("pt", new XAttribute("visit", "0"), new XAttribute("fid", fileId), new XAttribute("sl", i));
        ...
        {
            var unexecutedMethods =
                this.Report.Descendants("Type").Where(type => !type.Attribute("name").Value.Contains("__")).Elements("Method").Where(
                    m =>
                    !m.Attribute("name").Value.StartsWith("get_", StringComparison.Ordinal)
                    && !m.Attribute("name").Value.StartsWith("set_", StringComparison.Ordinal)).Where(m => !m.Elements().Any()).ToArray
                    ();

            long counter = 0;
            foreach (var method in unexecutedMethods)
            {
                var partCoverMethodElement = new PartCoverMethodElement(
                ...
                var fileIds =
                    method.Parent.Descendants("pt").Where(p => p.Attribute("fid") != null).Select(p => p.Attribute("fid").Value)
                        .Distinct().ToArray();
        ...
        private void AddCoverageDataOfAutoProperties(Dictionary<string, string> filenameByFileIdDictionary)
        {
            Func<string, bool> isProperty =
                v => v.StartsWith("get_", StringComparison.Ordinal) || v.StartsWith("set_", StringComparison.Ordinal);

            var unexecutedProperties =
                this.Report.Descendants("Type").Where(type => !type.Attribute("name").Value.Contains("__")).Elements("Method").Where(
                    m =>
                    isProperty(m.Attribute("name").Value)
                    && (!m.Elements().Any() || !m.Elements().Any(pt => pt.Attribute("sl") != null))).ToArray();
            ...
            foreach (var property in unexecutedProperties)
            {
                ...
                // Get files in which property could be defined
                var fileIds =
                    property.Parent.Descendants("pt").Where(p => p.Attribute("fid") != null).Select(p => p.Attribute("fid").Value)
                        .Distinct().ToArray();
