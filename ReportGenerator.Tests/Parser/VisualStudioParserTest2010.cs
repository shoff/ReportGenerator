namespace ReportGenerator.Tests.Parser
{
    ...
    [TestFixture]
    public class VisualStudioParserTest2010
    {
        private readonly string filePath = CommonNames.ReportDirectory + "VisualStudio2010.coveragexml";
        private ICollection<Assembly> assemblies;
        private VisualStudioParser visualStudioParser;

        [SetUp]
        public void SetUp()
        {
            var report = XDocument.Load(filePath);
            this.visualStudioParser = new VisualStudioParser(report);
            this.assemblies = this.visualStudioParser.Assemblies;
        }
        
        [Test]
        public void NumberOfLineVisitsTest()
        {
            var fileAnalysis = FileAnalysisCreator.GetFileAnalysis(assemblies, CommonNames.TestNamespace + "TestClass", CommonNames.CodeDirectory + "TestClass.cs");
            ...
            fileAnalysis = FileAnalysisCreator.GetFileAnalysis(assemblies, CommonNames.TestNamespace + "TestClass2", CommonNames.CodeDirectory + "TestClass2.cs");
            ...
            fileAnalysis = FileAnalysisCreator.GetFileAnalysis(assemblies, CommonNames.TestNamespace + "PartialClass", CommonNames.CodeDirectory + "PartialClass.cs");
            Assert.AreEqual(1, fileAnalysis.Lines.Single(l => l.LineNumber == 9).LineVisits, "Wrong number of line visits");
            Assert.AreEqual(0, fileAnalysis.Lines.Single(l => l.LineNumber == 14).LineVisits, "Wrong number of line visits");

            fileAnalysis = FileAnalysisCreator.GetFileAnalysis(assemblies, CommonNames.TestNamespace + "PartialClass", CommonNames.CodeDirectory + "PartialClass2.cs");
            Assert.AreEqual(1, fileAnalysis.Lines.Single(l => l.LineNumber == 9).LineVisits, "Wrong number of line visits");
            Assert.AreEqual(0, fileAnalysis.Lines.Single(l => l.LineNumber == 14).LineVisits, "Wrong number of line visits");
        }
        ...
        {
            Assert.AreEqual(1, assemblies.Single(a => a.Name == "ReportGenerator.Tests").Classes.Single(c => c.Name == CommonNames.TestNamespace + "TestClass").Files.Count());

            Assert.AreEqual(2, assemblies.Single(a => a.Name == "ReportGenerator.Tests").Classes
        ...
        {
            var metrics = assemblies.Single(a => a.Name == "ReportGenerator.Tests").Classes.Single(c => c.Name == CommonNames.TestNamespace + "TestClass").MethodMetrics;
